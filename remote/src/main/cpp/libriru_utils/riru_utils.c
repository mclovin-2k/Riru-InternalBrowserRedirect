...
int riru_utils_replace_native_functions(riru_utils_native_replace_t *functions, int length) {
    for (int i = 0; i < length; i++) {
        ...
        xhook_register(current->library_pattern, current->symbol, current->replace_function,
                       current->original_function);
    }
...
//Public Export
int
riru_utils_replace_jni_methods(riru_utils_jni_replace_method_t *classes, int length, JNIEnv *env) {
    for (int i = 0; i < length; i++) {
        riru_utils_jni_replace_method_t *current = classes + i;

        void *original_function = riru_api_v9->getJNINativeMethodFunc(riru_api_v9->token,
                                                                      current->class_name,
                                                                      current->method_name,
                                                                      current->signature);
        *current->original_function = original_function;
        riru_api_v9->setJNINativeMethodFunc(riru_api_v9->token, current->class_name,
                                            current->method_name, current->signature,
                                            current->replace_function);

        JNINativeMethod method = {current->method_name, current->signature,
                                  current->replace_function};
